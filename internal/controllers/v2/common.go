...
func removeFinalizer[T apiutils.Object](ctx context.Context, s store.Store[T], obj T, fin string) (T, bool, error) {
	...
	if err != nil {
		return obj, false, fmt.Errorf("failed to remove finalizer %q from %T: %w", fin, obj, err)
	}
	return updated, true, nil
}

func addFinalizer[T apiutils.Object](ctx context.Context, s store.Store[T], obj T, fin string) (T, bool, error) {
	if slices.Contains(obj.GetFinalizers(), fin) {
		return obj, false, nil
	}
	...
	updated, err := s.Update(ctx, obj)
	if err != nil {
		return obj, false, fmt.Errorf("failed to set finalizer %q on %T: %w", fin, obj, err)
	}
	return updated, true, nil
}
